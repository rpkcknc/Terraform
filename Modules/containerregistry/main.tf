...
resource "azurerm_container_registry" "container-registry1" {
name = var.contname
resource_group_name = var.contrg
location = var.contloc
sku = "Premium"
admin_enabled = true
network_rule_set = [ {
...
resource "azurerm_private_endpoint" "pvtend" {
    ...
    private_service_connection {
      name = var.pvt-svc
      private_connection_resource_id = azurerm_container_registry.container-registry1.id
